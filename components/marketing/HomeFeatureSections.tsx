  ...
  return (
    <Container>
      <div className={`grid items-center gap-10 lg:grid-cols-2 ${reverse ? 'lg:[&>*:first-child]:order-2' : ''}`}>
        <div>
          <p className="text-xs font-semibold uppercase text-[var(--color-primary-dark)]">{eyebrow}</p>
          <h3 className="mt-4 text-3xl font-bold leading-tight sm:text-4xl">{title}</h3>
          <p className="mt-5 leading-relaxed text-[var(--color-text-secondary)]">{desc}</p>
