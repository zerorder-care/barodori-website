...
const features = [
  { title: 'AI 기반 상태 측정', desc: '기울기, 가동범위, 두상 변화를 눈대중이 아닌 기록으로 확인' },
  { title: '맞춤형 운동 프로그램', desc: '사경 유형과 월령에 맞춘 코스형 가정 운동 루틴' },
  { title: '운동 기록 및 리포트', desc: '하루 운동과 측정 기록을 쌓아 아이의 변화를 차분하게 추적' },
  { title: '보호자 지원', desc: '커뮤니티, 챗봇, 정보 콘텐츠로 보호자의 막막함을 덜어주는 구조' },
] as const
        ...
        <div className="mt-12 grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
          {features.map((f, index) => (
            <article key={f.title} className="min-h-[360px] rounded-[8px] border border-[var(--color-border)] bg-white p-7">
              <div className="grid h-16 w-16 place-items-center rounded-[8px] bg-[#ededed] text-[10px] text-[var(--color-text-secondary)]">
                아이콘
              </div>
